...
public class EksportDoCSV2 extends JFrame {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					EksportDoCSV2 frame = new EksportDoCSV2(connection);
	 ...
	 * Create the frame.
	 */
	public EksportDoCSV2(final Connection connection) {
...
//		timeSpinner.setValue(new Date());
		...
		// pobranie dat z datechooserow
			...
			if(dataODString.equals("")||dataDOString.equals("")){
				...
				if(dataOD.compareTo(dataDO) > 0){
					...
					try {
					String query="INSERT INTO temporary2Export select access.id, access.id_karty, access.akcja, REPLACE (access.data, 'H', ' ') AS data FROM access WHERE data BETWEEN '"+dataODString+"' AND '"+dataDOString+"'";
					PreparedStatement pst=connection.prepareStatement(query);
					pst.execute();
					pst.close();
	...
	{
		try {
			String query="SELECT * FROM exportHistory ORDER BY ExportPlikuWdniu";
			PreparedStatement pst=connection.prepareStatement(query);
			ResultSet rs=pst.executeQuery();
			table.setModel(DbUtils.resultSetToTableModel(rs));
			pst.close();
